...
STACKING_POLICY = {
    ...
    "xiaomi-mimo": (
        "prohibited",
        'Platform terms: "Currently, the platform only supports purchasing 1 package at a time" and API keys are tied strictly to the individual subscribing account; proxying or non-coding API access is prohibited.',
    ),
    "anthropic-api": (
        "silent",
...
_CMD_PRICES = {
    ...
    "gemini 3.8 flash": {"input": 1.50, "output": 7.50, "cacheRead": 0.15, "cacheWrite": None},
    "gemini": {"input": 1.50, "output": 7.50, "cacheRead": 0.15, "cacheWrite": None},
}


def custom_model_pool(allowances, basis="list-price-credit", confidence="high"):
    out = {}
    for key, (allowance_usd, prices) in allowances.items():
        blended = agent_blend_cost(prices)
        out[key] = {
            "estimatedMillionTokens": round(allowance_usd / blended, 2),
            "basis": basis,
            "confidence": confidence,
        }
    return out
...
_CMD_GPT_POOL = _CMD_GO_POOL

_CMD_GOAT_POOL = custom_model_pool(
    {
        "gpt-5.6 sol": (70.0, _CMD_PRICES["gpt-5.6 sol"]),
        "glm-5.2": (70.0, _CMD_PRICES["glm-5.2"]),
        "tencent hy3": (70.0, _CMD_PRICES["tencent hy3"]),
        "qwen 3.8 27b": (70.0, _CMD_PRICES["qwen 3.8 27b"]),
        "deepseek v4 flash": (60.0, _CMD_PRICES["deepseek v4 flash"]),
        "minimax m3": (47.0, _CMD_PRICES["minimax m3"]),
        "glm-5.3 flash": (40.0, _CMD_PRICES["glm-5.3 flash"]),
        "gemini 3.8 flash": (40.0, _CMD_PRICES["gemini 3.8 flash"]),
        "gpt-5.6 luna": (20.0, _CMD_PRICES["gpt-5.6 luna"]),
        "qwen 3.8 max": (20.0, _CMD_PRICES["qwen 3.8 max"]),
        "grok 4.5": (20.0, _CMD_PRICES["grok 4.5"]),
    },
    confidence="high",
)

_CMD_PRO_POOL = custom_model_pool(
    {
        "gpt-5.6 sol": (80.0, _CMD_PRICES["gpt-5.6 sol"]),
        "glm-5.2": (80.0, _CMD_PRICES["glm-5.2"]),
        "tencent hy3": (80.0, _CMD_PRICES["tencent hy3"]),
        "qwen 3.8 27b": (80.0, _CMD_PRICES["qwen 3.8 27b"]),
        "deepseek v4 flash": (70.0, _CMD_PRICES["deepseek v4 flash"]),
        "minimax m3": (57.0, _CMD_PRICES["minimax m3"]),
        "glm-5.3 flash": (50.0, _CMD_PRICES["glm-5.3 flash"]),
        "gemini 3.8 flash": (50.0, _CMD_PRICES["gemini 3.8 flash"]),
        "gpt-5.6 luna": (30.0, _CMD_PRICES["gpt-5.6 luna"]),
        "qwen 3.8 max": (30.0, _CMD_PRICES["qwen 3.8 max"]),
        "grok 4.5": (30.0, _CMD_PRICES["grok 4.5"]),
        "claude sonnet 5": (20.0, _CMD_PRICES["claude sonnet 5"]),
        "claude opus 4.8": (20.0, _CMD_PRICES["claude opus 4.8"]),
        "claude haiku 4.5": (20.0, _CMD_PRICES["claude haiku 4.5"]),
        "gpt-5.6 terra": (20.0, _CMD_PRICES["gpt-5.6 terra"]),
        "gpt-5.5": (20.0, _CMD_PRICES["gpt-5.5"]),
    },
    confidence="high",
)

_CMD_MAX_10X_POOL = custom_model_pool(
    {
        # Standard models: $150 pool
        "gpt-5.6 sol": (150.0, _CMD_PRICES["gpt-5.6 sol"]),
        "deepseek v4 flash": (150.0, _CMD_PRICES["deepseek v4 flash"]),
        "glm-5.3 flash": (150.0, _CMD_PRICES["glm-5.3 flash"]),
        "gpt-5.6 luna": (150.0, _CMD_PRICES["gpt-5.6 luna"]),
        "minimax m3": (150.0, _CMD_PRICES["minimax m3"]),
        "grok 4.5": (150.0, _CMD_PRICES["grok 4.5"]),
        # Premium models: $100 pool
        "claude sonnet 5": (100.0, _CMD_PRICES["claude sonnet 5"]),
        "claude opus 4.8": (100.0, _CMD_PRICES["claude opus 4.8"]),
        "claude haiku 4.5": (100.0, _CMD_PRICES["claude haiku 4.5"]),
        "gpt-5.6 terra": (100.0, _CMD_PRICES["gpt-5.6 terra"]),
        "gpt-5.5": (100.0, _CMD_PRICES["gpt-5.5"]),
    },
    confidence="high",
)

_CMD_MAX_20X_POOL = custom_model_pool(
    {
        # Standard models: $300 pool
        "gpt-5.6 sol": (300.0, _CMD_PRICES["gpt-5.6 sol"]),
        "deepseek v4 flash": (300.0, _CMD_PRICES["deepseek v4 flash"]),
        "glm-5.3 flash": (300.0, _CMD_PRICES["glm-5.3 flash"]),
        "gpt-5.6 luna": (300.0, _CMD_PRICES["gpt-5.6 luna"]),
        "minimax m3": (300.0, _CMD_PRICES["minimax m3"]),
        "grok 4.5": (300.0, _CMD_PRICES["grok 4.5"]),
        # Premium models: $200 pool
        "claude sonnet 5": (200.0, _CMD_PRICES["claude sonnet 5"]),
        "claude opus 4.8": (200.0, _CMD_PRICES["claude opus 4.8"]),
        "claude haiku 4.5": (200.0, _CMD_PRICES["claude haiku 4.5"]),
        "gpt-5.6 terra": (200.0, _CMD_PRICES["gpt-5.6 terra"]),
        "gpt-5.5": (200.0, _CMD_PRICES["gpt-5.5"]),
    },
    confidence="high",
)
...
write_json(
    "claude-code.json",
    {
        ...
        "tiers": [
            ...
            },
            {
                ...
                "limits": {
                    ...
                    "weeklyAllocation": "Weekly sliding window (~6x Pro empirical ceiling, not 20x)",
                ...
                "estimatedTokenBudget": {
                    "description": "Weekly-capped steady state (~72M agentic tokens/mo, 20x burst)",
                    "estimatedMillionTokens": 72,
                    "midpointEstimate": 144,
                    "optimisticEstimate": 240,
                    "assumptions": "While Max 20x provides 20x headroom in rolling 5h sessions (up to 240M peak), empirical telemetry confirms the weekly sliding window caps sustained monthly throughput to ~6x Pro (~72M tokens/mo conservative, 144M midpoint, 240M theoretical unconstrained burst ceiling)",
                },
                "perModelTokenBudgets": _claude_pool(72, include_fable=True),
        ...
        "gotchas": [
            ...
            "While Max 20x provides 20x headroom within 5-hour burst windows, empirical telemetry (Faros AI / Morph) confirms the overall weekly ceiling scales to only ~6x of the Pro tier, causing high-velocity developers to hit weekly account lockouts prematurely",
            "In May 2026, Anthropic doubled 5-hour rate limits for Pro/Max/Team and eliminated peak-hour throttling; weekly hard caps still enforce account ceilings",
            "Claude Code capacity is pooled across CLI sessions, Claude.ai web/desktop apps, and Claude Cowork",
...
write_json(
    "windsurf.json",
    {
        ...
        "gotchas": [
            ...
            "On March 19, 2026, Windsurf replaced credit blocks with auto-refreshing daily and weekly quotas integrated with Devin",
        ...
        "tiers": [
            {
                ...
                "monthlyPrice": 1,
                "limits": {
                    "monthlyCredits": "$10/mo compute credits ($1/mo plan fee)",
                    "fiveHourCap": "$3 rolling limit (resets 5h from first use)",
                    "weeklyCap": "$6 rolling limit (resets 7d from first use)",
                    "requests": "~15K mix estimate (~26K with DeepSeek V4 Flash at typical cache)",
                    "topUp": "Extra pay-as-you-go credits are never limited and never expire",
                },
                "models": [
                    "DeepSeek V4 Flash",
                    "GPT-5.6 Luna",
                ...
                "perModelTokenBudgets": dict(_CMD_GO_POOL),
                "estimatedTokenBudget": {
                    "description": "$10 compute credits (~15M tokens on open models)",
                    "estimatedMillionTokens": 15,
                    "midpointEstimate": 26,
                    "optimisticEstimate": 84,
                    "assumptions": "$10.00 / ~$0.65/M blended open-model rate ~= 15M tokens; vendor benchmarks ~26K requests on DeepSeek V4 Flash (~26M tokens) and up to 84M on GPT-5.6 Luna",
                    "estimateMeta": {
                        "sourceUrl": "https://commandcode.ai/docs/resources/pricing-limits#usage-estimates",
                        "sourceQuote": "On Go, DeepSeek V4 Flash runs ~ 42K requests with no cache - ~ 26K once the typical 50K cache reads are included. Our plan estimate is the mix average across the plan's typical models.",
                        "sourceType": "official",
                        "confidence": "high",
                        "verifiedAt": LAST_VERIFIED,
                        "basisModel": "open-model-mix",
                    },
                },
                "notes": "Processing fee applies; 46 open models + select premium (GPT-5.6 Luna, Grok 4.5, Qwen Max/Plus)",
            },
            {
                "name": "GOAT",
                "monthlyPrice": 10,
                "limits": {
                    "monthlyCredits": "$70/mo usage value credits ($10/mo plan fee)",
                    "fiveHourCap": "$14 rolling limit (resets 5h from first use)",
                    "weeklyCap": "$35 rolling limit (resets 7d from first use)",
                    "requests": "~75K mix estimate (~154K with DeepSeek V4 Flash)",
                    "modelAllowances": "$70 Sol/GLM-5.2/Hy3/Qwen27B · $60 DeepSeek Flash · $47 MiniMax M3 · $40 GLM Flash · $20 Luna/Grok",
                    "topUp": "Extra pay-as-you-go credits are never limited and never expire",
                },
                "models": [
                    "DeepSeek V4 Flash",
                ...
                "estimatedTokenBudget": {
                    ...
                    "midpointEstimate": 120,
                    "optimisticEstimate": 751,
                    "assumptions": "$70.00 / ~$1/M blended ~= 70M tokens; official vendor model allowances range from $20 to $70 per model ($60 on DeepSeek V4 Flash delivers 751M tokens; ~154K requests)",
                    "estimateMeta": {
                        "sourceUrl": "https://commandcode.ai/docs/plans/goat",
                        "sourceQuote": "You pay $10 and code with up to $70 of per-model credit allowances... $14 of usage in any 5 hours, $35 in any 7 days, and $70 per month.",
                        "sourceType": "official",
                        "confidence": "high",
                        "verifiedAt": LAST_VERIFIED,
                        "basisModel": "GLM-5.2",
                ...
                "notes": "49+ models; includes official per-model dollar allowances up to $70",
            },
            {
                "name": "Pro",
                "monthlyPrice": 20,
                "limits": {
                    ...
                    "fiveHourCap": "$16 rolling limit (resets 5h from first use)",
                    "weeklyCap": "$40 rolling limit (resets 7d from first use)",
                    "requests": "~100K mix estimate (~47% standard / ~53% premium)",
                    "modelAllowances": "$80 Sol/GLM-5.2/Hy3 · $70 DeepSeek Flash · $57 MiniMax M3 · $50 GLM Flash · $20 Claude Sonnet/Opus",
                    "topUp": "Extra pay-as-you-go credits are never limited and never expire",
                },
                "models": [
                    "Claude Sonnet 5",
                ...
                "estimatedTokenBudget": {
                    ...
                    "midpointEstimate": 120,
                    "optimisticEstimate": 877,
                    "assumptions": "$80.00 / ~$1/M blended ~= 80M tokens; official vendor model allowances range from $20 for premium frontier models (Claude Sonnet/Opus, Terra) up to $80 for standard models ($70 on DeepSeek V4 Flash delivers 877M tokens)",
                    "estimateMeta": {
                        "sourceUrl": "https://commandcode.ai/docs/plans/pro",
                        "sourceQuote": "Pro works the same way: 16 credits in any 5 hours and 40 in any 7 days across its per-model credits - premium models included - worth $16 / $40 of usage on a full-allowance model.",
                        "sourceType": "official",
                        "confidence": "high",
                        "verifiedAt": LAST_VERIFIED,
                        "basisModel": "GLM-5.2",
                ...
                "notes": "Adds all premium models incl Claude Opus/Sonnet; 1M context",
            },
            {
                "name": "Max 10x",
                "monthlyPrice": 100,
                "limits": {
                    "standardPool": "$150/mo standard model usage limit (Sol, Luna, Grok, DeepSeek)",
                    "premiumPool": "$100/mo premium model usage limit (Claude Opus, Sonnet, Fable)",
                    "fiveHourCap": "$45 rolling limit (resets 5h from first use)",
                    "weeklyCap": "$90 rolling limit (resets 7d from first use)",
                    "requests": "~230K mix estimate (~43% standard / ~57% premium)",
                    "topUp": "Extra pay-as-you-go credits are never limited and never expire",
                },
                "models": [
                    "Claude Sonnet 5",
                ...
                "perModelTokenBudgets": dict(_CMD_MAX_10X_POOL),
                "estimatedTokenBudget": {
                    "description": "$150 standard + $100 premium compute credits",
                    "estimatedMillionTokens": 150,
                    "midpointEstimate": 230,
                    "optimisticEstimate": 500,
                    "assumptions": "Max 10x provides dual pools: $150 standard model usage limit + $100 premium model usage limit (~230K mix requests); standard models include Sol, Luna, Grok, DeepSeek; premium models include Claude Opus, Sonnet, Fable",
                    "estimateMeta": {
                        "sourceUrl": "https://commandcode.ai/docs/plans/max#usage-limits",
                        "sourceQuote": "On Max 10×, you have a $150 standard model usage limit and a $100 premium model usage limit... $45 of usage in any 5 hours and $90 in any 7 days.",
                        "sourceType": "official",
                        "confidence": "high",
                        "verifiedAt": LAST_VERIFIED,
                        "basisModel": "dual-pool-mix",
                    },
                },
                "notes": "Dual pools: $150 standard + $100 premium model usage; full catalog",
            },
            {
                "name": "Max 20x",
                "monthlyPrice": 200,
                "limits": {
                    "standardPool": "$300/mo standard model usage limit (Sol, Luna, Grok, DeepSeek)",
                    "premiumPool": "$200/mo premium model usage limit (Claude Opus, Sonnet, Fable)",
                    "fiveHourCap": "$90 rolling limit (resets 5h from first use)",
                    "weeklyCap": "$180 rolling limit (resets 7d from first use)",
                    "requests": "~370K mix estimate (~43% standard / ~57% premium)",
                    "topUp": "Extra pay-as-you-go credits are never limited and never expire",
                },
                "models": [
                    "Claude Sonnet 5",
                ...
                "perModelTokenBudgets": dict(_CMD_MAX_20X_POOL),
                "estimatedTokenBudget": {
                    "description": "$300 standard + $200 premium compute credits",
                    "estimatedMillionTokens": 300,
                    "midpointEstimate": 370,
                    "optimisticEstimate": 1000,
                    "assumptions": "Max 20x provides dual pools: $300 standard model usage limit + $200 premium model usage limit (~370K mix requests); standard models include Sol, Luna, Grok, DeepSeek; premium models include Claude Opus, Sonnet, Fable",
                    "estimateMeta": {
                        "sourceUrl": "https://commandcode.ai/docs/plans/max#usage-limits",
                        "sourceQuote": "On Max 20×, that's $300 standard and $200 premium... $90 of usage in any 5 hours and $180 in any 7 days.",
                        "sourceType": "official",
                        "confidence": "high",
                        "verifiedAt": LAST_VERIFIED,
                        "basisModel": "dual-pool-mix",
                ...
                "notes": "Dual pools: $300 standard + $200 premium model usage; highest request volume",
            },
        ],
        "gotchas": [
...
_OPENCODE_GO_PRICES = {
    "kimi k3": (15.0, {"input": 3.00, "output": 15.00, "cacheRead": 0.30, "cacheWrite": None}),
    "qwen 3.8 max": (15.0, {"input": 2.00, "output": 6.00, "cacheRead": 0.25, "cacheWrite": 2.50}),
    "grok 4.6": (15.0, {"input": 2.00, "output": 6.00, "cacheRead": 0.50, "cacheWrite": None}),
    "qwen 3.7 max": (30.0, {"input": 2.50, "output": 7.50, "cacheRead": 0.50, "cacheWrite": 3.125}),
    "glm-5.3": (15.0, {"input": 1.40, "output": 4.40, "cacheRead": 0.26, "cacheWrite": None}),
    "glm-5.2": (60.0, {"input": 1.40, "output": 4.40, "cacheRead": 0.26, "cacheWrite": None}),
    "glm-5.1": (60.0, {"input": 1.40, "output": 4.40, "cacheRead": 0.26, "cacheWrite": None}),
    "deepseek v4 pro": (15.0, {"input": 0.66, "output": 1.98, "cacheRead": 0.022, "cacheWrite": None}),
    "kimi k2.6": (60.0, {"input": 0.95, "output": 4.00, "cacheRead": 0.16, "cacheWrite": None}),
    "kimi k2.7 code": (60.0, {"input": 0.95, "output": 4.00, "cacheRead": 0.19, "cacheWrite": None}),
    "hy4 preview": (30.0, {"input": 0.834, "output": 2.501, "cacheRead": 0.042, "cacheWrite": None}),
    "gpt-5.6 luna": (15.0, {"input": 0.20, "output": 1.20, "cacheRead": 0.02, "cacheWrite": 0.25}),
    "minimax m3": (60.0, {"input": 0.30, "output": 1.20, "cacheRead": 0.06, "cacheWrite": None}),
    "mimo-v2.5-pro": (15.0, {"input": 0.435, "output": 0.87, "cacheRead": 0.003625, "cacheWrite": None}),
    "qwen 3.6 plus": (60.0, {"input": 0.50, "output": 3.00, "cacheRead": 0.05, "cacheWrite": 0.625}),
    "minimax m2.7": (60.0, {"input": 0.30, "output": 1.20, "cacheRead": 0.06, "cacheWrite": 0.375}),
    "minimax m2.5": (60.0, {"input": 0.30, "output": 1.20, "cacheRead": 0.06, "cacheWrite": 0.375}),
    "qwen 3.7 plus": (60.0, {"input": 0.40, "output": 1.60, "cacheRead": 0.04, "cacheWrite": 0.50}),
    "hy3": (60.0, {"input": 0.14, "output": 0.58, "cacheRead": 0.035, "cacheWrite": None}),
    "qwen 3.8 flash": (30.0, {"input": 0.15, "output": 0.47, "cacheRead": 0.016, "cacheWrite": 0.20}),
    "glm-5.3 flash": (60.0, {"input": 0.15, "output": 0.50, "cacheRead": 0.03, "cacheWrite": None}),
    "deepseek v4 flash vision exp": (15.0, {"input": 0.15, "output": 0.60, "cacheRead": 0.003, "cacheWrite": None}),
    "longcat-2.0": (60.0, {"input": 0.30, "output": 1.20, "cacheRead": 0.006, "cacheWrite": None}),
    "deepseek v4 flash": (30.0, {"input": 0.15, "output": 0.60, "cacheRead": 0.003, "cacheWrite": None}),
    "deepseek v4.1 flash": (60.0, {"input": 0.15, "output": 0.60, "cacheRead": 0.003, "cacheWrite": None}),
    "mimo-v2.5": (60.0, {"input": 0.14, "output": 0.28, "cacheRead": 0.0028, "cacheWrite": None}),
    "muse spark 1.3 contributor": (60.0, {"input": 0.10, "output": 0.20, "cacheRead": 0.002, "cacheWrite": None}),
    "muse spark 1.2 contributor": (60.0, {"input": 0.10, "output": 0.20, "cacheRead": 0.002, "cacheWrite": None}),
}
_OPENCODE_GO_POOL = custom_model_pool(_OPENCODE_GO_PRICES, confidence="high")
    ...
    "opencode.json",
    {
        ...
        "tiers": [
            ...
            },
            {
                ...
                "monthlyPrice": 10,
                "limits": {
                    "monthlyAllowance": "Per-model monthly usage: $60 (tier 1 models), $30 (tier 2), $15 (tier 3)",
                    "fiveHourCap": "20% of monthly allowance in any 5h ($12 / $6 / $3)",
                    "weeklyCap": "50% of monthly allowance in any 7d ($30 / $15 / $7.50)",
                    "monthlyCap": "100% of monthly allowance ($60 / $30 / $15)",
                    "requests": "Up to 31,580/mo on GLM-5.3-Flash, 65,000/mo on DeepSeek Flash, 150,400/mo on MiMo-V2.5",
                    "topUp": "Optional 'Use balance' falls back to Zen PAYG balance when limits reached",
                },
                "models": [
                    "Kimi K3",
                    "Qwen 3.8 Max",
                ...
                "perModelTokenBudgets": _OPENCODE_GO_POOL,
                "estimatedTokenBudget": {
                    "description": "$10/mo open-model subscription ($15–$60 per-model allowances; ~30M–752M tokens)",
                    "estimatedMillionTokens": 30,
                    "midpointEstimate": 95,
                    "optimisticEstimate": 752,
                    "assumptions": "$10/mo plan fee provides independent monthly usage allowances per model ($15, $30, or $60) with 20% 5h and 50% weekly caps; tokens range from 7.85M (Kimi K3 at $15 allowance) to 94M (Kimi K2.7 Code at $60) up to 752M (DeepSeek V4.1 Flash at $60 promo) and 981M (MiMo-V2.5)",
                    "estimateMeta": {
                        "sourceUrl": "https://opencode.ai/docs/go/#usage-limits",
                        "sourceQuote": "Usage limits are defined as monthly dollar amounts... Each model has the following usage limits: 5-hour — 20% of the monthly limit; weekly — 50%; and monthly — 100%. For example, if a model has a $60 monthly limit, you can spend up to: 5-hour limit — $12, weekly limit — $30, monthly limit — $60.",
                        "sourceType": "official",
                        "confidence": "high",
                        "verifiedAt": LAST_VERIFIED,
                        "basisModel": "open-model-allowances",
                    },
                },
                "notes": "New low-cost open-model subscription with independent allowances per model",
            },
        ],
        "gotchas": [
            "OpenCode Go ($10/mo) provides $15, $30, or $60 monthly usage allowance per model with 20% 5h and 50% weekly caps",
            "Optional 'Use balance' falls back to Zen PAYG balance when Go model limits are reached",
            "DeepSeek V4.1 Flash has 4× promotional limits ($60/mo limit) through Sep 20, 2026 (normally $15)",
        ...
        "tosHighlights": [
            "Zero data retention on most Go models (0 days); 30 days on Grok 4.6 and GPT-5.6 Luna; Muse Spark Contributor trains on code",
            "Zen models US-hosted with zero retention",
        ],
        "dataTraining": "Zero data retention (0 days) on most models; 30 days on Grok 4.6 & GPT-5.6 Luna; Muse Spark Contributor trains on code",
        "ipIndemnity": false,
    },
)
        ...
        "tiers": [
            {
                ...
                "limits": {
                    ...
                    "weeklyCeiling": "Up to 97M GLM-5.3 / 584M GLM-5.3-Flash tokens/week (~420M / 2,529M monthly ceiling)",
                    "weeklyLimitGLM53": "Up to 97M Tokens/week",
                    "weeklyLimitGLM53Flash": "Up to 584M Tokens/week",
                    "offPeakDiscount": "50% credit rate during off-peak",
                    "peakHours": "Mon-Fri 14:00-18:00 UTC+8",
                    "concurrency": "Recommended for 1 project at a time (dynamic off-peak boost)",
                ...
                "perModelTokenBudgets": {
                    ...
                    "glm-5.3-flash": {
                        ...
                        "midpointEstimate": 1264,
                        "optimisticEstimate": 2529,
                        "basis": "official-table",
                        "confidence": "high",
                    },
                },
            ...
            },
            {
                ...
                "limits": {
                    ...
                    "weeklyCeiling": "Up to 582M GLM-5.3 / 3,504M GLM-5.3-Flash tokens/week (~2,520M / 15,172M monthly ceiling; 6× Lite)",
                    "weeklyLimitGLM53": "Up to 582M Tokens/week",
                    "weeklyLimitGLM53Flash": "Up to 3,504M Tokens/week",
                    "offPeakDiscount": "50% credit rate during off-peak",
                    "concurrency": "Recommended for 1-2 projects simultaneously (dynamic off-peak boost)",
                ...
                "perModelTokenBudgets": {
                    ...
                    "glm-5.3-flash": {
                        ...
                        "midpointEstimate": 7600,
                        "optimisticEstimate": 15172,
                        "basis": "official-table",
                        "confidence": "high",
                    },
                },
            ...
            },
            {
                ...
                "limits": {
                    ...
                    "weeklyCeiling": "Up to 1,358M GLM-5.3 / 8,176M GLM-5.3-Flash tokens/week (~5,884M / 35,402M monthly ceiling; 14× Lite)",
                    "weeklyLimitGLM53": "Up to 1,358M Tokens/week",
                    "weeklyLimitGLM53Flash": "Up to 8,176M Tokens/week",
                    "offPeakDiscount": "50% credit rate during off-peak",
                    "priority": "Priority resource allocation during peak hours",
                    "concurrency": "Recommended for 2+ projects simultaneously (highest allocation, dynamic off-peak boost)",
                },
                "models": ["GLM-5.3", "GLM-5.3-Flash"],
                "perModelTokenBudgets": {
                    "glm-5.3": {
                    ...
                    "glm-5.3-flash": {
                        ...
                        "midpointEstimate": 17731,
                        "optimisticEstimate": 35402,
                        "basis": "official-table",
                        "confidence": "high",
                    },
                },
        ...
        "tiers": [
            {
                ...
                "annualPrice": 63.36,
                "limits": {
                    "credits": "4,100,000,000 (4.1B) Credits/mo",
                    "concurrency": "Standard dynamic concurrency",
                    "rpm": "100 RPM across models",
                    "tpm": "10,000,000 TPM (10M)",
                    "models": "6 models (mimo-v2.5-pro, mimo-v2.5, mimo-v2.5-asr, TTS series)",
                },
                "models": [
                    "MiMo-V2.5-Pro",
                    "MiMo-V2.5",
                ...
                "perModelTokenBudgets": {
                    "mimo-v2.5-pro": {
                        "estimatedMillionTokens": 40.28,
                        "midpointEstimate": 45.0,
                        "optimisticEstimate": 50.35,
                        "basis": "official-table",
                        "confidence": "high",
                    },
                    "mimo-v2.5": {
                        "estimatedMillionTokens": 117.94,
                        "midpointEstimate": 130.0,
                        "optimisticEstimate": 147.43,
                        "basis": "official-table",
                        "confidence": "high",
                    },
                },
                "estimatedTokenBudget": {
                    "description": "4.1B Credits/mo; ~40M tokens on MiMo-V2.5-Pro or ~118M on MiMo-V2.5 (official credit conversion)",
                    "estimatedMillionTokens": 40.28,
                    "midpointEstimate": 79.11,
                    "optimisticEstimate": 117.94,
                    "assumptions": "4.1B Credits / 101.79M credits/M tokens on MiMo-V2.5-Pro (standard 20:1 + 75% cache) ~= 40.28M tokens; on MiMo-V2.5 (34.76M credits/M) ~= 117.94M tokens. Nighttime 0.8x off-peak rate grants +25% capacity bonus",
                    "estimateMeta": {
                        "sourceUrl": "https://mimo.mi.com/docs/en-US/price/token-plan",
                        "sourceQuote": "Lite: $6/month, ¥39/month, 4,100,000,000 Credits. mimo-v2.5-pro: 2.5 cache hit, 300 cache miss, 600 output; mimo-v2.5: 2 hit, 100 miss, 200 out",
                        "sourceType": "official",
                        "confidence": "high",
                        "verifiedAt": LAST_VERIFIED,
                        "basisModel": "MiMo-V2.5-Pro",
                        "cacheAssumption": "75% cache hit rate (official 2.5 Credits hit vs 300 miss)",
                    },
                },
                "notes": "Lite tier for developers; compatible with OpenCode, OpenClaw, Claude Code, Cline, and Codex via OpenAI/Anthropic proxy protocols",
            },
            {
                "name": "Standard",
                "monthlyPrice": 16,
                "annualPrice": 168.96,
                "limits": {
                    "credits": "11,000,000,000 (11B) Credits/mo",
                    "concurrency": "Standard dynamic concurrency",
                    "rpm": "100 RPM across models",
                    "tpm": "10,000,000 TPM (10M)",
                    "models": "6 models (mimo-v2.5-pro, mimo-v2.5, mimo-v2.5-asr, TTS series)",
                ...
                "perModelTokenBudgets": {
                    "mimo-v2.5-pro": {
                        "estimatedMillionTokens": 108.07,
                        "midpointEstimate": 120.0,
                        "optimisticEstimate": 135.09,
                        "basis": "official-table",
                        "confidence": "high",
                    },
                    "mimo-v2.5": {
                        "estimatedMillionTokens": 316.44,
                        "midpointEstimate": 350.0,
                        "optimisticEstimate": 395.55,
                        "basis": "official-table",
                        "confidence": "high",
                    },
                },
                "estimatedTokenBudget": {
                    "description": "11B Credits/mo; ~108M tokens on MiMo-V2.5-Pro or ~316M on MiMo-V2.5",
                    "estimatedMillionTokens": 108.07,
                    "midpointEstimate": 212.26,
                    "optimisticEstimate": 316.44,
                    "assumptions": "11B Credits / 101.79M credits/M tokens on MiMo-V2.5-Pro ~= 108.07M tokens; on MiMo-V2.5 ~= 316.44M tokens",
                    "estimateMeta": {
                        "sourceUrl": "https://mimo.mi.com/docs/en-US/price/token-plan",
                        "sourceQuote": "Standard: $16/month, ¥99/month, 11,000,000,000 Credits",
                        "sourceType": "official",
                        "confidence": "high",
                        "verifiedAt": LAST_VERIFIED,
                        "basisModel": "MiMo-V2.5-Pro",
                ...
                "notes": "Standard package for regular AI programming workloads",
            },
            {
                "name": "Pro",
                "monthlyPrice": 50,
                "annualPrice": 528.00,
                "limits": {
                    "credits": "38,000,000,000 (38B) Credits/mo",
                    "concurrency": "High dynamic concurrency",
                    "rpm": "100 RPM across models",
                    "tpm": "10,000,000 TPM (10M)",
                    "models": "6 models (mimo-v2.5-pro, mimo-v2.5, mimo-v2.5-asr, TTS series)",
                },
                ...
                "perModelTokenBudgets": {
                    "mimo-v2.5-pro": {
                        "estimatedMillionTokens": 373.33,
                        "midpointEstimate": 420.0,
                        "optimisticEstimate": 466.66,
                        "basis": "official-table",
                        "confidence": "high",
                    },
                    "mimo-v2.5": {
                        "estimatedMillionTokens": 1093.15,
                        "midpointEstimate": 1200.0,
                        "optimisticEstimate": 1366.44,
                        "basis": "official-table",
                        "confidence": "high",
                    },
                },
                "estimatedTokenBudget": {
                    "description": "38B Credits/mo; ~373M tokens on MiMo-V2.5-Pro or ~1.09B on MiMo-V2.5",
                    "estimatedMillionTokens": 373.33,
                    "midpointEstimate": 733.24,
                    "optimisticEstimate": 1093.15,
                    "assumptions": "38B Credits / 101.79M credits/M tokens on MiMo-V2.5-Pro ~= 373.33M tokens; on MiMo-V2.5 ~= 1,093.15M tokens (1.09B)",
                    "estimateMeta": {
                        "sourceUrl": "https://mimo.mi.com/docs/en-US/price/token-plan",
                        "sourceQuote": "Pro: $50/month, ¥329/month, 38,000,000,000 Credits",
                        "sourceType": "official",
                        "confidence": "high",
                        "verifiedAt": LAST_VERIFIED,
                        "basisModel": "MiMo-V2.5-Pro",
                ...
                "notes": "Pro package for daily high-frequency agentic software development",
            },
            {
                "name": "Max",
                "monthlyPrice": 100,
                "annualPrice": 1056.00,
                "limits": {
                    "credits": "82,000,000,000 (82B) Credits/mo",
                    "concurrency": "Maximum dynamic concurrency",
                    "rpm": "100 RPM across models",
                    "tpm": "10,000,000 TPM (10M)",
                    "models": "6 models (mimo-v2.5-pro, mimo-v2.5, mimo-v2.5-asr, TTS series)",
                },
                ...
                "perModelTokenBudgets": {
                    "mimo-v2.5-pro": {
                        "estimatedMillionTokens": 805.61,
                        "midpointEstimate": 900.0,
                        "optimisticEstimate": 1007.01,
                        "basis": "official-table",
                        "confidence": "high",
                    },
                    "mimo-v2.5": {
                        "estimatedMillionTokens": 2358.90,
                        "midpointEstimate": 2600.0,
                        "optimisticEstimate": 2948.63,
                        "basis": "official-table",
                        "confidence": "high",
                    },
                },
                "estimatedTokenBudget": {
                    "description": "82B Credits/mo; ~806M tokens on MiMo-V2.5-Pro or ~2.36B on MiMo-V2.5",
                    "estimatedMillionTokens": 805.61,
                    "midpointEstimate": 1582.26,
                    "optimisticEstimate": 2358.90,
                    "assumptions": "82B Credits / 101.79M credits/M tokens on MiMo-V2.5-Pro ~= 805.61M tokens; on MiMo-V2.5 ~= 2,358.90M tokens (2.36B). Off-peak nighttime 0.8x rate reaches over 1.0B on Pro and 2.95B on V2.5",
                    "estimateMeta": {
                        "sourceUrl": "https://mimo.mi.com/docs/en-US/price/token-plan",
                        "sourceQuote": "Max: $100/month, ¥659/month, 82,000,000,000 Credits",
                        "sourceType": "official",
                        "confidence": "high",
                        "verifiedAt": LAST_VERIFIED,
                        "basisModel": "MiMo-V2.5-Pro",
                ...
                "notes": "Max package for high-intensity engineering teams and autonomous agent loops",
            },
        ],
        "gotchas": [
            "Strictly restricted to AI programming scenarios (OpenCode, OpenClaw, Claude Code, Cline, Codex); generic API scraping or non-coding backends are prohibited and subject to API key revocation",
            "Platform allows only 1 active subscription package per account; upgrades are supported by paying the pro-rata price difference, but downgrading is disallowed",
            "When the monthly Credit quota is exhausted, service halts safely without automatic overdraft or balance depletion",
            "MiMo-V2.5-Pro-UltraSpeed (>1,000 TPS TileRT engine) is not included in the Token Plan quota and must be accessed via PAYG",
            "Nighttime off-peak rate: UTC 16:00-24:00 (Beijing 0:00-8:00) consumes credits at 0.8x coefficient (+25% yield bonus)",
            "Unused credits do not roll over to subsequent months and are non-refundable once activated",
        ],
        "tosHighlights": [
            "Includes regional proxy clusters in Singapore, Europe (Amsterdam), and China with native OpenAI and Anthropic compatibility",
            "TTS voice synthesis models are free for a limited time and do not deplete Token Plan credits",
            "Zero balance drain: service cutoffs prevent surprise charges on underlying account balance",
        ],
        "dataTraining": "Standard platform terms apply to consumer subscriptions; zero data retention and enterprise isolation available under enterprise contracts",
        "ipIndemnity": false,
    },
)

print("All 34 plans generated successfully!")
